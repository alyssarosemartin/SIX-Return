...
#Download 1981-2019 SI-x bloom Anomalies, using a loop - will only get 2019 after it is addded as PRISM
...
SIXA20 <- npn_download_geospatial("si-x:bloom_anomaly","2020-05-29") 
NAvalue(SIXA20) <- -9999
SIXA20_4k <-resample(SIXA20, SIXA_stack, method='bilinear') 
plot(SIXA20_4k)

#Create an "On-time" 2020 raster where all on-time pixels are = to 1.
...
writeRaster(SIXA20_4k_OnTime, "2020_Anom_OnTime_bloom.tiff", format="GTiff",  overwrite=TRUE, NAflag=-9999) 
...
writeRaster(RI_Early, "2020_RI_Early_Bloom.tiff", format="GTiff", overwrite=TRUE, NAflag=-9999)
...
#Plot example later than current year map to check.
plot(SIXA_stack_Later[[20]],
...
writeRaster(RI_Late, "2020_Anom_Late_bloom.tiff", format="GTiff",  overwrite=TRUE, NAflag=-9999) 
